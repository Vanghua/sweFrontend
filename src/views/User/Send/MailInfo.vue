    ...
    <a-form :form="form" @submit="submitAll" :label-col="{xs: { span: 24 }, sm: { span: 4 }}" :wrapper-col="{ xs: { span: 24 }, sm: { span: 16 },}">
      ...
      <a-form-item label="选择地址">
        <a-card>
          ...
          <div id="mailInfoMap" style="height: 300px; border: 1px solid #d9d9d9; border-radius: 15px; width: 100%;"></div>
        </a-card>
      </a-form-item>
      <a-form-item>
        ...
        <a-button type="danger" @click="$router.push({name: 'Send', params: { mailInfo: mailInfo, acceptInfo: acceptInfo }})">返回</a-button>
      </a-form-item>
    </a-form>
  </div>
</template>
...
// 引入自动填写模态窗口
import AutoMailInfo from "@/views/User/Send/AutoMailInfo";

export default {
  name: "MailInfo",
  components: {
    'auto-Modal': AutoMailInfo
  },
  mounted() {
    // 数据规范化
    ...
    if(JSON.stringify(this.$route.params) != JSON.stringify({})) {
      this.form.setFieldsValue({
        'name': this.$route.params.mailInfo.name,
        'phone': this.$route.params.mailInfo.phone,
        'address': this.$route.params.mailInfo.address,
        'addressDetail': this.$route.params.mailInfo.addressDetail,
      })
  ...
  methods: {
    ...
    autoMail(record) {
      this.isAuto = false
      this.form.setFieldsValue({
        'name': record.mailName,
        'phone': record.mailPhone,
        'address': record.mailAddress,
        'addressDetail': record.mailAddressDetail,
      })
      this.setPlace(record.mailAddressDetail)
    },
    ...
    initMap() {
      // 实例化地图
      this.map = new BMap.Map("mailInfoMap");
      // 地图中心点
